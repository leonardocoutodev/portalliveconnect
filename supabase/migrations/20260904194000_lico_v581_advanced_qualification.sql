-- Live Connect V5.8.1 — Lico: qualificação comercial aprofundada + identidade Comercial no chat interno
...
alter table public.commercial_chat_sessions add column if not exists guardian_required boolean;
alter table public.commercial_chat_sessions add column if not exists guardian_is_contact boolean;
alter table public.commercial_chat_sessions add column if not exists guardian_name text;
alter table public.commercial_chat_sessions add column if not exists guardian_whatsapp text;
...
alter table public.commercial_chat_sessions add column if not exists preferred_class_id uuid;
...
begin
 ...
 select x.id,x.channel_id,x.sender_id,coalesce(p.full_name,'Equipe Live Connect'),
   case when exists(select 1 from private.system_owner o where o.user_id=x.sender_id) then 'admin_comercial' else p.role::text end,
   x.body,x.reply_to_id,x.metadata,x.created_at,(x.sender_id=auth.uid())
 from (select m.* from public.school_chat_messages m where m.channel_id=p_channel_id and m.deleted_at is null order by m.created_at desc limit greatest(1,least(coalesce(p_limit,80),200))) x
 left join public.profiles p on p.id=x.sender_id
 order by x.created_at asc;
end $$;

revoke all on function public.school_commercial_chat_sessions(integer,text) from public,anon,authenticated;
...
begin
 ...
 select x.id,x.sender_type,x.sender_id,
   case when x.sender_type='visitor' then 'Lead' when x.sender_type='assistant' then 'Lico' else coalesce(p.full_name,'Equipe Live Connect') end,
