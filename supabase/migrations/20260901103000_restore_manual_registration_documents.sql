-- Admin manual registration + automatic documents
-- Applied in production on 2026-09-01

CREATE OR REPLACE FUNCTION public.admin_get_contract(p_contract_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  select to_jsonb(c) into v_out from public.contracts c where c.id=p_contract_id;
  if v_out is null then raise exception 'contract_not_found' using errcode='P0002'; end if;
  return v_out;
end
$function$
;
...
begin
  ...
  if v_kind='young_apprentice' then
    ...
    v_snapshot:=jsonb_build_object(
      ...
      'available_shift',v_shift,'project','Projeto Jovem Aprendiz','submitted_at',now(),
    ...
    values(v_lead_id,'inscricao_jovem_aprendiz_manual','Projeto Jovem Aprendiz — ficha criada manualmente no Admin',
           jsonb_build_object('form_id',v_form,'interest_id',v_interest));
