...
function cors(req:Request){const o=req.headers.get('origin')||'';return {'Access-Control-Allow-Origin':okOrigin(o)&&o?o:'https://www.liveconnect.com.br','Vary':'Origin','Access-Control-Allow-Headers':'authorization, content-type, apikey, x-client-info','Access-Control-Allow-Methods':'GET, POST, OPTIONS','Cache-Control':'no-store'}}
const services:Record<string,string>={enrollment:'portal-enrollment-submit',lead:'portal-lead-submit',young:'portal-young-apprentice-submit',analytics:'portal-analytics-event',payment:'portal-payment',ouro:'ouro-student-portal',dkweb:'dkweb-student-portal'};
...
Deno.serve(async req=>{
 ...
 if(req.method==='GET'&&u.searchParams.get('selftest')==='1'){
   const results={
     ...
     dkweb:await probe(services.dkweb,{action:'summary'})
   };
   const expected=results.enrollment.status===400&&results.lead.status===400&&results.young.status===400&&results.analytics.status===400&&[400,404].includes(results.payment.status)&&results.ouro.status===401&&results.dkweb.status===401;
   return new Response(JSON.stringify({ok:expected,origin:'https://www.liveconnect.com.br',services:results}),{status:expected?200:500,headers:{'Content-Type':'application/json; charset=utf-8',...cors(req)}});
 }
